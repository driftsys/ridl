...
| `just doc-path-check`  | every `docs/` file path named in a tracked file resolves — the bare paths `link-check` cannot see, in prose, in an inline code span and in a source comment. A directory citation is not checked. Skips `docs/archive/` and `docs/wip/`; a path belonging to another repository has to be written as a URL |
