...
| `just doc-path-check`  | every `docs/` file path named in a tracked file resolves — the bare paths `link-check` cannot see, in prose, in an inline code span and in a source comment. A directory citation is not checked, and a file that is not text is named rather than scanned. Skips `docs/archive/` and `docs/wip/`; a path belonging to another repository has to be written as a URL |
...
| `just demo`            | generate the cabin example's crate and run the program that links it — each round trip's value is matched, and a missing one or a non-zero exit fails                                                                                                                                                                                                                |
| `just build`           | `toolchain-check` + `gate-parity` + `install-check` + `fmt-check` + `book-check` + `link-check` + `doc-path-check` + `compile` + `test` + `lint` + `wasm-check` + `compat-check` + `demo` + `check` — the full local gate                                                                                                                                            |
