...
| `just msrv-check`      | build and test `ridl-rt` with its minimum supported Rust version (ADR-0021 decision 10)                                                                                                     |
| `just build`           | `toolchain-check` + `gate-parity` + `install-check` + `fmt-check` + `book-check` + `link-check` + `compile` + `test` + `lint` + `wasm-check` + `msrv-check` + `check` — the full local gate |
