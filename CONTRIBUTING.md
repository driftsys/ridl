...
| `just build`           | `toolchain-check` + `gate-parity` + `install-check` + `fmt-check` + `book-check` + `link-check` + `compile` + `test` + `lint` + `wasm-check` + `compat-check` + `check` — the full local gate |
