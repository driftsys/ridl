...
//! is tested rather than a list of examples. A port of
//! `crates/ridl-backend-proto/tests/stability.rs` — the property statement is
//! identical, and only the parser over the emitted text differs, because this
//! target assigns a field's number as an `(id: N)` attribute rather than as
//! proto3's `= N;`.
//!
...
/// table or enum so two declarations cannot share a key. A number that moves
/// between two schemas is exactly a change to this map's values.
///
/// Two line shapes assign a number on this target. A table field carries an
/// `(id: N)` attribute — `limit: uint (id: 2);`, with `= null` or
/// `, deprecated` present on some fields — and a retired-slot placeholder is
/// such a field too: unlike proto3's nameless `reserved 3;`, it binds a name
/// (`reserved_3`) to an id, so it is read like any other field. An enum body
/// line binds a value name to its number — `V0 = 0,` — in a declared enum
/// and in a generated identity table alike.
///
/// A comment line is skipped before anything else: an enum set inlines as a
/// scalar under a `// B0 = bit 0` comment per bit, which would otherwise
/// read as a value assignment.
///
...
fn assigned_numbers(schema: &str) -> std::collections::BTreeMap<String, i64> {
    ...
    for line in schema.lines() {
        ...
        if let Some(rest) = line.strip_prefix("enum ") {
            // `enum Mode : long {` — the name stops at the underlying type.
            scope = rest.split(':').next().unwrap_or("").trim().to_string();
            in_enum = true;
            continue;
        }
        if line == "}" {
        ...
        // The namespace line, an include, and a one-line `union` declaration
        // (which assigns no numbers) all sit outside any scope.
        if scope.is_empty() {
            continue;
        }
        if in_enum {
            ...
            let Some((name, digits)) = without_comma.split_once('=') else {
                continue;
            };
            let Ok(number) = digits.trim().parse::<i64>() else {
                continue;
            };
            out.insert(format!("{scope}.{}", name.trim()), number);
        } else {
            // `limit: uint (id: 2);`, `mode: Mode = null (id: 3);`,
            // `reserved_3: ubyte (id: 2, deprecated);` — the number is the
            // digits after `(id:`, the name is what stands before the `:`.
            let Some(clause) = line.find("(id:") else {
                continue;
            };
            let digits: String = line[clause + "(id:".len()..]
                .trim_start()
                .chars()
...
proptest! {
    ...
    fn a_compatible_change_never_moves_an_assigned_number(
        ...
        let old_numbers = assigned_numbers(
            &ridl_backend_flatbuffers::generate(&old).expect("generate old").fbs_source
        );
        let new_numbers = assigned_numbers(
            &ridl_backend_flatbuffers::generate(&new).expect("generate new").fbs_source
        );
