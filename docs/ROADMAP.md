...
alone, and the two bindings by name: E11.9's WebSocket transport and, on
Android, a runtime's own binder contract. ridl specifies no Binder layout and no
transaction code; this reverses the lane P driver's decision D-P5, which had the
Kotlin backend generate an AIDL binding per interface (driftsys/ridl#516). No
binding is written yet, and nothing in this workspace speaks the frame;
`ridl-loopback` runs in process.
...
| ID     | Story                                                                                                                                                                                                                                                                                                                                   | Done when                                                                                                                                                                                                                                                                                                                                                             | Size |
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| E11.16 | `ridl-rt`: a keyed `Wakeable` port extension — `wake_on(what: Interest, waker: &core::task::Waker)`, one waker per kind of key per handle, woken by a change to any key of that kind — and `Transport::Busy`, the remote "busy, try later" relayed as a call's outcome                                                                  | `ridl-loopback` implements `Wakeable` and its tests wake each handle's waiter on a change to any key of its kind; `Transport::Busy` is added without a breaking change                                                                                                                                                                                                | S    |
...
| E11.18 | `ridl-rt`: `correlate::Table<const N: usize>` — slots with a generation counter, a stored outcome and one waker, the correlation `(generation << 16) \| slot`, a byte budget per member reservation, every `Slot` waiter woken on a reclaim — and the per-kind waker registry behind `Wakeable`; caller-supplied storage, no allocation | unit tests cover slot reuse, a generation mismatch, the `Slot` waiters, the byte budget, and `forget` freeing a slot                                                                                                                                                                                                                                                  | M    |
...
| E11.20 | `ridl-rt-conformance`: the port contract tests as functions generic over a runtime factory with a clock hook and a fault hook, so any runtime runs them from its own test suite                                                                                                                                                         | `ridl-loopback` runs the suite from the crate and passes, a named mutation in the loopback turns it red, and every test that stayed in the loopback is listed with its reason; once E11.16 and E11.18 land, the suite covers `Wakeable` (one waker per kind of key per handle, woken by any key of that kind) and the table's behaviour as observed through the ports | M    |
...
That change has landed (driftsys/ridl#516), and it reverses the lane P driver's
decision D-P5 in all three texts.
...
**The Kotlin runtime owns its IPC binding.** On Android, the Kotlin runtime
binds the ports over its own binder contract, which may be one generic,
versioned AIDL serving every catalog; the Kotlin backend generates no binding,
and ridl specifies no Binder layout and no transaction code
([the frame specification](specification/frame-specification.md) §11.2). This
reverses the lane P driver's decision D-P5, which had the Kotlin backend
generate an AIDL binding per interface (driftsys/ridl#516). E11.9's WebSocket
transport is not on Kotlin's path.
