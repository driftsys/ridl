...
| E9.7  | The **projection contract** and its pinned name transform — the collision rule (RIDL-149) replaces the injectivity requirement; the divergent `interact.rs` implementation is deleted in favour of `c_header.rs`'s; specified with E4.5's stability policy (ADR-0016) | one transform, in `ridl-ir`; a package whose names collide after it is rejected                                                         | M    |
