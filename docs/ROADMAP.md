...
| E15.3 | `ridl lock` — allocation, `--rename`, `--retire` — `ridl lock merge` as the git merge driver, and the `ridl check` desk check that runs while RIDL-409 stands and names the command that fixes it (plan Tasks 5, 6, 7)         | `ridl lock` allocates a number for a new interface, a merge of two branches keeps both sides' numbers, and the rename hint names one command | L    |
...
| E15.6 | The records: the ADR-0015 and ADR-0016 amendments, the decisions index, ridl reference §11, §14.5, §16.4 and Appendix B, the CLI reference and ADR-0010's rows for `ridl lock` (plan Task 12)                                  | no record still describes a service slot, an implicit interface id, or numbering by position                                                 | S    |
