...
Read with it: `docs/wip/2026-09-13-lock-design.md` (interface numbers, B4) and
`docs/wip/2026-09-13-runtime-descriptors-design.md` (what a descriptor needs
...
| `crates/ridlc/tests/corpus.rs`                  | B3 Tasks 2 to 9; B4 Tasks 6, 9   | S2 (the RIDL-408 row) → L4 (not listed for B3) |
...
**Appendix A's package name.** Appendix A declares `package veh.system`, and
`system` is a reserved word in every profile (typl §1.4), which no
`qualified_id` segment may be (typl Appendix E: `id` excludes reserved words).
The parser rejects `package veh.system` with FORM-101 in every profile, before
this plan and after. The test sources below quote Appendix A with the package
renamed `veh.topology`. The executor posts this difference on #328 (Global
Constraints); the reference is not changed by this task.
...
```rsdl
// rsdl reference Appendix A, verbatim except the package: Appendix A writes
// `package veh.system`, and `system` is a reserved word, which no package
// segment may be (typl §1.4, Appendix E `qualified_id`).
package veh.topology
...
/// rsdl reference Appendix A, `system.rsdl`, with the package renamed:
/// `system` is a reserved word and cannot be a package segment (typl §1.4).
...
mod tests {
    ...
    fn a_reference_reads_its_role_from_case() {
        ...
        assert_eq!(
            form("veh.system.Cruise"),
            declared(Some("veh.system"), "Cruise")
        );
        assert_eq!(
            form("veh.system.Cruise.primary"),
            instance(Some("veh.system"), "Cruise", "primary")
        );
        assert_eq!(
...
**Shared file.** `crates/ridlc/tests/corpus.rs` (lanes plan §6: S2 → L4; B3 is
not listed). Step 1 checks it.
...
`crates/ridlc/tests/corpus.rs` (S2 → L4). Step 1 checks both.
...
- **The tests read their examples from the rsdl reference**, as Task 8 reads
  §16.1. They rely on the Appendix A fix this plan's pull request makes
  (`package veh.topology`): `package veh.system` is FORM-101 in every profile,
  because `system` is reserved.
