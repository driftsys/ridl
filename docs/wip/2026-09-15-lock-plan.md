...
- **PD-21 — the records lane B handed to lane L.** Lane B's stage B1 comment on
  ...
  rsdl §5.3, RSDL-502 and declared redundancy, and its RSDL-801/RSDL-803
  citation; ADR-0016's three "rsdl §13" citations; the rsdl reference §13
  sentence that says ridl §11 and §14.5 number an interface by position until
  the lock lands; the vocabulary note's V-17, V-X2 and §7 `lock` line; the
  family general form §6.3. Design §11 placed general form §6.3 outside this
  lane, but it was written before that comment, and no other lane owns the item.
  Task 12 edits each of these where the lock or the rsdl reference v0.2.0 makes
  its text false, and leaves it unchanged otherwise.
...
| `docs/specification/ridl-family-overview.md`             | 12                | S2 → B1 → **L4** → L5: one line of the open-question index (§17.14)                                          |
...
- Produces, in `ridl_core::interface_lock` (no re-export at the crate root:
  ...
  - `impl InterfaceLock { pub fn render(&self) -> String; pub fn live(&self, key: &LockKey) -> Option<&LockEntry>; pub fn max_number(&self) -> u32; pub fn allocate(&mut self, key: LockKey) -> Result<u32, LockEditError>; pub fn rename(&mut self, old: &LockKey, new: LockKey) -> Result<u32, LockEditError>; pub fn retire(&mut self, key: &LockKey) -> Result<u32, LockEditError> }`.
    `allocate` returns a `Result`, not the skeleton's bare `u32`: a key that is
    already live is `KeyAlreadyLive`. `render` writes `HEADER`, `next N`, then
    every entry in number order, one `\n` after each line (PD-7).
...
- Modify: `crates/ridl-sem/src/check.rs` (the `use` list; `check_package`: the
  ...
  field of the `v2::Package` literal; four new items after `check_package` —
...
- Modify: `crates/ridlc/tests/corpus.rs` (`compile_entry`: the `render_ids` — a
  third remapper, which PD-12 does not list; `RIDL_PROFILE_CODES`; the
  `EXPECTED` table in `showcase_pins_every_severity`)
...
No test pins the reference; `just build` (`prim lint`, `link-check`) is its
gate. In `docs/specification/ridl-language-reference.md` §11, replace the
...
this task; the frame's `heading` rule (a
`#[deny(clippy::wildcard_enum_match_arm)]` on `heading` too) and the design's
"the IR `Reserved` slot still exists in this task and the set walk ignores it".
...
§14.5 as a set, §16.4 rows "retired by the lock", Appendix A), §12 bullet 7 (the
...
- Modify: `docs/specification/ridl-language-reference.md` (§14.5, §16.4,
  Appendix A), `docs/book/getting-started.md` (one paragraph)
...
Expected after Task 8 (matching lines per file):

- `RIDL-146`: `crates/ridl-core/src/diag.rs` 2, `crates/ridl-sem/src/check.rs`
  11, `crates/ridlc/tests/corpus.rs` 2,
  `…/ridl-diag-showcase/main/services.ridl` 1,
  `…/corpus__diagnostics@ridl-diag-showcase.snap` 1, `docs/decisions/ADR-0015-…`
  4, `docs/specification/ridl-language-reference.md` 2.
- `RIDL-147`: `diag.rs` 1, `check.rs` 11, `corpus.rs` 2, showcase `NOTES` 1,
  `main/services.ridl` 2, `twin/twin.ridl` 2, the diagnostics snapshot 1,
  ADR-0015 2, ADR-0016 2, the ridl reference 2.
- `RIDL-148`: `diag.rs` 2, `crates/ridl-ir/proto/ridl/ir/v2/ir.proto` 1,
  `check.rs` 6, `corpus.rs` 2, `main/services.ridl` 2, the diagnostics snapshot
  1, ADR-0015 1, `docs/decisions/README.md` 1, the ridl reference 3.
- `RIDL_146`/`RIDL_147`/`RIDL_148`: `diag.rs` 1 each; `check.rs` 1, 2, 1.
- The second grep: `crates/ridl-core/src/package.rs` (the `ServiceShape::` arms
  and a fixture line), `crates/ridl-ir/src/lib.rs` (two `Kind::Reserved` arms, a
  test fixture, `slot.id` in a test), `crates/ridl-lsp/src/hover.rs`,
  `crates/ridl-lsp/tests/server.rs`, `crates/ridl-sem/src/check.rs`,
  `crates/ridl/src/main.rs` (two `ServiceShape::` arms),
  `crates/ridl/tests/baseline_gate.rs`, `crates/ridlc/tests/corpus.rs`,
  `crates/ridlc/tests/corpus/services-workspace/vehicle/publish.ridl`.
...
3. In §16.4, the three rows keep their code and are marked retired; their
   ...
   ```markdown
   | RIDL-146 | retired by the lock (2026-09-15; ADR-0015 decision 24 as amended) — was: interface re-declared under a service-level `reserved` name; a service's list holds no tombstone | retired |
...
4. In Appendix A, replace the two productions
...
**Model:** fable. **Depends on:** Task 1, Task 4, Task 8. **Design:** §7
"Decision — matching" (`diff_interfaces` keys on the number, inline shapes
included; a matched pair goes to `diff_interface` as today; the classifier
re-finds the old side by number), §7 rows 1 to 8 of the table, §7 "the heading"
...
- Differences from the skeleton's T9: `diff_interfaces` takes the two packages,
  not shape slices and retired lists; there is no `InterfaceKey` enum and no
  `find_interface_pair` — the pass structure and `find_old_shape` replace them.
...
**Model:** fable. **Depends on:** Task 4, Task 10. **Design:** §4 (the table's
`ridl baseline` column: row 1, row 5's first publication, row 6's two phases),
§8 (the RIDL-411 and RIDL-412 rows; "inside the gate PR #330 adds between the
clean compile and the publication: exit 1, the staging directory removed, every
published file byte-identical"; RIDL-411 reads the fresh IR alone, RIDL-412 the
same `diff_sets` report the RIDL-408 gate walks), §11 (the ADR-0010 row, the
§16.4 rows), §12 bullet 7. **Plan decisions:** PD-9 (RIDL-412 fires only for a
...
- Modify: `docs/book/cli-reference.md` (`### ridl baseline`: the opening
  paragraph and the publication-gate paragraph; the exit-code table's
  `ridl baseline` row)
...
- Produces:
  ...
  - The test helpers `lock(root: &Path)` in `baseline_desk.rs` and
    ...
    `baseline_gate.rs`.
- Differences from the skeleton's T10: one function, `interface_refusals`,
  replaces `provisional_interfaces` and `dropped_numbers`; the RIDL-411 span is
  the declaration's name rather than detached; no severity rows.

- [ ] **Step 1: Declare the two codes**

The string-literal guard (`codes_written_as_string_literals_are_all_catalogued`
in `crates/ridl-core/src/diag.rs`) fails on a `"RIDL-411"` literal anywhere in a
`.rs` file before the code is in the catalogue, so the declaration comes before
the tests that name it. In `crates/ridl-core/src/diag.rs`, inside
`RIDL_CATALOG`, after the `RIDL_410` entry (which ends
`"`interfaces.lock`is malformed";`) and before the catalogue's closing brace,
add:
...
ADR-0015 decision 9's rsdl citations and its RSDL-801/803 citation; ADR-0016's
three "rsdl §13" citations; the rsdl reference §13 sentence; the vocabulary
note's V-17, V-X2 and §7 `lock` line; the family general form §6.3 — each edited
where the lock or the rsdl reference v0.2.0 makes its text false, and left
unchanged otherwise).

Nothing executable changes. No test pins a record; `just build` — `prim lint` at
its floor tier, `link-check`, `book-check` — is the gate. Every edit below is
...
- **The sweep found one line outside the frame's shared-files table.**
  `docs/specification/ridl-family-overview.md` indexes ridl §17's open
  questions, and its §17.14 line repeats the wording Task 11 corrected; the
  frame lists the overview under "none". Step 4 fixes that one line and the
  driver reports it as a deviation from the table.
...
- Modify: `docs/specification/ridl-language-reference.md` (Appendix B, the
  `ordinals (§11)` row; §16.4's RIDL-141/RIDL-143 sentence; Appendix A's
  `service_def` comment)
- Modify: `docs/specification/ridl-family-overview.md` (the §17.14 line of the
  open-question index — the sweep's one hit)
...
3. Decision 3. The text
   `RIDL-149 is the direct sibling of RIDL-147, which [ADR-0015](ADR-0015-qos-absorption-and-rpc-bounds.md) decision 24 minted for interface names colliding within a service; this is the same fail-closed rule one level down.`
   becomes:
...
- Fixed by this task: `docs/decisions/ADR-0016-…:132`
  ...
  `docs/specification/ridl-language-reference.md` §16.4's
  `RIDL-141 and RIDL-143 apply **per shape in the service's shape list**`, which
  becomes `apply **per interface in the service's list**`, and Appendix A's
  `service_def` comment `(* the shape list — §14.5; commas are`, which becomes
  `(* the list, a set — §14.5; commas are`; and
  `docs/specification/ridl-family-overview.md`'s open-question index, where
  ...
  (the line Task 11's §17.14 rewrite made stale; the frame's shared-files table
  lists the overview under "none" — reported as a deviation).
- Unchanged, the retired model recorded as history under a dated amendment:
  every `slot`, `shape list`, `ServiceShape`, `tombstone` and `append-only` hit
  inside ADR-0015 decisions 12, 15, 19 and 24 and its `## Status` note.
- Unchanged, about interaction ordinals or composite fields, not the service
  ...
  `:318`, the exit-code table's "tombstone rule";
  ...
  `docs/specification/ridl-family-overview.md:96`, `:191`, `:202`;
