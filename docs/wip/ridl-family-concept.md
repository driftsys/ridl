...
| **ridl** | reactive interface description language | system interactions — `signal` `event` `command` `query` `final` | service teams                 | descriptive (contract)   |
...
| provisioned constant   | `final`            | static capability    |
...
- **Language Reference** — §3–§5 are re-framed as **typl**; the interaction
  sections gain a note that `signal`/`event`/`command`/`query`/`final` are the
