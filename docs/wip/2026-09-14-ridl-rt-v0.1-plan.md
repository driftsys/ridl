...
in dependency order: `encoding` ← `payload` ← `sample` and `error` ← `contract`
← `port`. Every trait a generated package or a runtime implements is exercised
from an integration test, which is a separate crate, so the tests use the crate
the way other crates will. The last task is the E11.0 program: a hand-written
catalog, descriptor, codec, in-memory runtime and accessor.
...
- [ ] **A2 merged.** `gh pr list --state merged --search "ridl-rt plan"` lists
      the pull request that carries this file.
...
- [ ] **The state G2 leaves.** `grep -n "fourteen crates" AGENTS.md` prints one
      line and `grep -n '"ridl-mcp"' .git-std.toml` prints one line. Task 1's
      registration edits are written against that state. If either prints
...
| C-5 | "A `compile_fail` doctest shows ... (E0451)" (§5)                   | each `compile_fail` doctest has a doctest beside it that compiles, with the same imports and signature                                                                                                                                                                 | rustdoc on 1.95.0 does not check the error code: with `E0308` in place of `E0451` the doctest still passed. The compiling doctest shows the paths resolve, so the failing one fails for the reason it states.                                                                 |
...
| C-9 | the driver prompt names the crate list and count in `AGENTS.md`     | also the crate tree in `README.md`, the scope list in `CONTRIBUTING.md`, and the comment above `wasm-check` in the `justfile`                                                                                                                                          | the first two list every crate or scope and S1 updated both for `ridl-mcp`; the comment explains which crates the recipe covers                                                                                                                                               |
...
`.git-std.toml`, `justfile`, `AGENTS.md`, `README.md`, `CONTRIBUTING.md`. The
root `Cargo.toml` is not edited: its `members = ["crates/*", "xtask"]` includes
the new directory.
...
```text
# `ridl-rt` is included because generated Rust links it on wasm32 (ADR-0020
# decision 5).
```
...
Expected: `just build` passes. `git diff --stat` shows the six modified files
...
**Implementer:** Fable. **Spec:** R-8; RA-08 and RA-21 as §4 of the spec words
them.
...
/// A stand-in second codec: the same little-endian bytes as above, with a
/// parsed view instead of a byte view. Only the view differs.
...
- [ ] **Shared files (lanes plan §6).** Check that no other open pull request
  ...
  gh pr list --state open --json number,title,files --jq '.[]
    | select(any(.files[].path; IN("Cargo.toml","Cargo.lock",".git-std.toml","AGENTS.md","justfile","README.md","CONTRIBUTING.md")))
    | "\(.number) \(.title)"'
  ```
