...
- [ ] **The state G2 leaves.** Each of these prints one line:
      ...
      Task 1's registration edits are written against that state. If one prints
      nothing, stop and report it.
...
| C-9 | the driver prompt names the crate list and count in `AGENTS.md`     | also the crate tree in `README.md`, the scope list in `CONTRIBUTING.md`, the comment above `wasm-check` in the `justfile`, an amendment to ADR-0007 decision 14, and one comment in `crates/ridl/tests/version.rs`                                                     | the first two list every crate or scope and S1 updated both for `ridl-mcp`; the `justfile` comment explains which crates the recipe covers; after G2, ADR-0007 decision 14 states "The workspace crates stay at `0.0.0`" and the `version.rs` comment states that every workspace crate shares `0.0.0`, and a crate at 0.1.0 makes both false (the version itself is P-1 and R-12, decided by Sebastien) |
...
`docs/decisions/ADR-0007-e1-execution.md` and `crates/ridl/tests/version.rs`.
The root `Cargo.toml` is not edited: its `members = ["crates/*", "xtask"]`
...
(`docs/wip/2026-09-13-ridl-rt-v0.1-design.md`).
```
...
Expected: `just build` passes. `git diff --stat` shows the eight modified files
...
/// `MAX_SIZE` so the last test can tell the two constants apart.
...
- [ ] **Shared files (lanes plan §6).** Check that no other open pull request
  ...
  gh pr list --state open --json number,title,files --jq '.[]
    | select(any(.files[].path; IN("Cargo.toml","Cargo.lock",".git-std.toml","AGENTS.md","justfile","README.md","CONTRIBUTING.md","docs/decisions/ADR-0007-e1-execution.md","crates/ridl/tests/version.rs")))
    | "\(.number) \(.title)"'
  ```
...
- **Stage A4** (branch `chore/ridl-rt-release`), from R-12: the crates.io
  ...
  with the tag `ridl-rt@0.1.0`; and `sdd-gardening` over the spec and this plan,
