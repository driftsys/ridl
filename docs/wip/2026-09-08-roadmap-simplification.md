...
requirement leans on hardest is downstream of the language S-25 defers. Rule
numbers are cited by
...
**Goal B — one system's SSOT.** The xflux view-if bus in HASC is the only
consumer that exists. It needs typl types with their constraints, ridl
interfaces, a stable IR, the two wire projections, the trait layer that
...
## 3. Principles
    ...
    P-1  One consumer orders the backlog. A story xflux does not need, and
...
## 4. The cut
    ...
    S-04  NARROW  E11.1 from a frame and control-plane specification to the
                  port and descriptor contract. The frame is a wire; xflux
                  has its own and needs no second one. What E11.1's exit
    ...
    S-23  ADD     **SPIKE-1 — the activation model.** What a service's step
                  ...
                  note is not ridl's to write and not on this plan. xflux
                  needs it now, because xflux is building the thing; ridl
                  needs it when rmdl lands. It moves to the xflux repo and
    ...
    S-29  ADD     **The stabilisation loop, as the finish test for typl and
                  ...
                  that has a subject — author the HASC view-if catalog
                  (`com.ampere.ccs.viewif`, a real contract with real
                  interfaces) as a corpus package, compile it, and record
    ...
    S-30  CONFIRM **The generated proto3/FlatBuffers gateway is held off.**
                  ...
                  encodes with its own adapters (xflux already has
                  `har-signal`'s `Pod` and `Fb`). That is a coherent
    ...
    S-35  ADOPT   **The descriptor uses the vocabulary that is settled.**
                  The HASC topology naming of 2026-08-10 — **machine**
                  ...
                  integration: nobody discovers at run time whether HASC is
                  on the QNX host. The one consequence to carry across:
    ...
    S-37  RECORD  **Offer/consume and placement must stay in separate
                  ...
                  interface does **not** change between HASC v1 and v2;
    ...
    S-38  AMEND   **ridl §14 must say whether a `service` runs.** Today it
                  ...
                  execution the way the HASC vocabulary already does
    ...
    S-39  SHAPE   **Offer a service; consume an interface.** The two sides
                  ...
                  HASC rule that an interface has exactly one owning
    ...
    S-42  SHAPE   **Hash per interface, not per catalog.** ADR-0016
                  ...
                  **interface**, which is already the unit ADR-0018 §8
                  ...
                  move the HASC vocabulary made for `machine_id`: **a
    ...
    S-50  SETTLE  **One unit, one job — and no unit holds two.** The test
                  ...
                  either (S-46). In the C-EV cockpit all of these happen to
...
xflux, it is this repository. That is a legitimate second consumer and it is
...
The cost is stated plainly rather than argued away: with `ridl-rt` parked,
**xflux is the only implementation of the seven ports**, and RA-02 and RA-03
claim they are defined once and implementable by any runtime. A trait layer
validated by one implementation is shaped by that implementation. The partial
mitigation is that the Kotlin client is a second _binding_ of the same ports
across a language boundary (xflux API-36, API-37), which finds a different class
of defect than a second Rust runtime would, and finds it sooner.
...
generates — the collision the HASC naming pass recorded on 2026-08-10 when it
rejected the word, and a second collision with AUTOSAR Classic's own meaning
...
settles the containment question the same way the HASC naming pass did: three
machines, on one piece of hardware.
...
the whole of it fits on a page:
    ...
    system Cockpit                       // the closure
      ...
      service hasc   offers   Hasc, Cluster
                     consumes Disco
      service ckui   consumes Hasc        // an interface, not a service
    ...
    deployment V1 for Cockpit            // placement, region two
      ...
      place hasc  on aaos_ivi
      place ckui  on aaos_ivi

    deployment V2 for Cockpit
      ...
      place hasc  on qnx_host            // the only line that differs
...
HASC vocabulary already fixes **one interface, exactly one owning service**, so
interface -> service -> machine resolves without ambiguity. That rule is what
makes consuming an interface well defined at all, and it is already settled.
...
the real C-EV topology, then decide.** A week of authoring says more about
whether this wants a grammar, diagnostics and an LSP than any amount of arguing
here, and the four concepts above are small enough that either answer is cheap.
What must not happen is the third option — a TOML file that grows a grammar one
field at a time without anyone deciding it did.
...
In the HASC vocabulary a VM is a **machine**, and `vm` was rejected as the word
precisely because the QNX host is a machine too and does not sit under the
hypervisor. So: a VM is a machine; the cockpit is the system; `host` and `guest`
are roles a machine holds relative to the hypervisor, and HASC changes sides at
v2 without its machine changing what it is. Any descriptor that conflates the
first two will need re-authoring at v2, which is the concrete cost of getting
this wrong.
...
Four items, because a cut that only subtracts hides the work it displaced.
    ...
    A-2  `ridl-backend-kotlin`, sized L, on the critical path.
         ...
         **ADR-0018 decision 6 narrowed language scope to Rust, TypeScript
         ...
         ladder, calling it "a real target". Rung 3 is where xflux's client
         runs, and that client is what makes the seven ports a contract
...
> interfacing rules for each reading machine, against `ridl-rt`. The xflux
...
E13's trigger is a deployed system with two encodings, which is what a HASC v2
with a CAN-facing service would be — so the gateway is parked rather than
dismissed. The `ridl-engine` row is the one that changed character between the
two revisions: it was a document this plan owed, and it is now a language this
plan has deliberately deferred, which is a longer wait and a clearer one.
...
kept because xflux's remote leg is proto3, but that leg is a v2 concern in HASC
and the v1 path is FlatBuffers in shared memory. If the remote leg slips, E11.8
(L) parks with it and the release loses 4.5 weeks and its byte-level conformance
story. Decide with the xflux v1/v2 split, not here.
...
**SR-X5 — the second consumer.** P-1 rests on xflux being the only one. If a
second appears inside the cut's horizon, the ordering is re-derived rather than
patched, because two consumers with different needs is the situation that
produced §2's tension in the first place.
...
ports turn out to be xflux-shaped, the discovery comes late.

**SR-X7 — whether SPIKE-1 is one note or two, now that it is xflux's.** §4
argues the sans-IO activation question and rmdl's synchronous step are one
question seen from two ends. S-23 moved the note to xflux, which needs the first
half now and does not need the second at all. If they are one question, xflux
answers it and ridl inherits the answer for free when rmdl lands; if they are
two, xflux answers a smaller one and ridl still owes the rest. Either way this
plan pays nothing, which is what makes S-23 the cheap move — but it also means
nobody is watching whether the two halves agree.
...
unit, as the HASC vocabulary says it is. Answer that by writing the first
...
its first version: with one SoC in the C-EV cockpit, every crossing is either
intra-machine or hypervisor, and the distinction has no case to decide yet. Add
it when a second box appears, and keep the field name aligned with the
