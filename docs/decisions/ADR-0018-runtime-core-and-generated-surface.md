...
**Amendment (2026-09-12) — the re-scope, and what `ridl-rt` names.** The
2026-09-12 re-scope changed five of these decisions; each change is written into
decisions 3, 6, 15, 16 and 17 below. Decisions 3, 6 and 15 rest on
[ADR-0020](ADR-0020-third-encoding-runtime-layering-and-plugin-system.md);
decision 16 rests on the two-step plan of [`docs/ROADMAP.md`](../ROADMAP.md) and
decision 17 on the re-scope note's §3.1, neither of which is that record's
material. One correction applies to the whole record. **`ridl-rt` here names the
engine; everywhere else it now names the library.** Decision 1's layer 2 and
decision 2's sans-IO core are the engine — the store, the seqlock, the
scheduler, the ring depth, the control plane and the platform traits — which
[`docs/wip/2026-09-08-ridl-rt-design.md`](../wip/2026-09-08-ridl-rt-design.md)
§0 moved out of this repository and [`docs/ROADMAP.md`](../ROADMAP.md) parks as
`ridl-engine`, reopened by rmdl. The name `ridl-rt` now belongs to the portable
library of contract descriptors, ports, samples and encodings that generated
code links and a runtime implements (ADR-0020 decision 5). Decisions 1 and 2
stand as written, about `ridl-engine`; nothing in them binds the library. This
discharges that note's RA-X10.

Not yet swept: the ridl, rmdl and rsdl references still cite `ridl-rt` for a
scheduler, supervision hooks, acks and quarantine, which are `ridl-engine`'s.
Those references are unimplemented or under rewrite, so each sweep waits for the
...
3. **Two encodings and no more: proto3 on the network, FlatBuffers in memory.**
   ...
   **Amendment (2026-09-12) — three encodings, and the wasm boundary divided.**
   [ADR-0020](ADR-0020-third-encoding-runtime-layering-and-plugin-system.md)
   decision 1 adds `repr(C)` as a third payload encoding, for a consumer that
   reads a field at a fixed offset and links no reader library, so "two
   encodings and no more" no longer holds. The rule that classifies a boundary
   is unchanged, and ADR-0020 decision 2 carries the matrix all three sit in.

   ADR-0020 decision 2 also divides "the wasm guest boundary", which this
   decision states as one case, into the two it holds. A codec compiled to wasm
   from the same IR as the TypeScript that calls it has no version skew to
   survive, so by the rule above it is a within-node boundary and carries
   **FlatBuffers**. proto3 stays on the boundary of a guest that updates
   independently of its host, which is decision 14's case and the one the
...
6. **Language scope narrows to Rust, TypeScript and Kotlin.** Rust is primary,
   ...
   **Amendment (2026-09-12) — Kotlin leaves the workspace, and its codec is not
   reached by JNI.**
   [ADR-0020](ADR-0020-third-encoding-runtime-layering-and-plugin-system.md)
   decisions 8 to 11 make a language backend an executable over a published
   contract, so Kotlin is generated by an out-of-tree `ridlc-gen-kotlin` plugin
   rather than by a backend in this workspace. Two clauses above are withdrawn
   with it: that Kotlin receives generated types and interfaces only, and that
   its codec is the Rust one reached via JNI.

   **The Kotlin codec is not the Rust one over JNI.**
   [`docs/wip/2026-09-08-ridl-rt-design.md`](../wip/2026-09-08-ridl-rt-design.md)
   §9 found that JNI cannot serve a mapped read path at all — a JNI transition
   per read costs two orders of magnitude more than the mapped load it wraps,
   which is that note's RA-30 — and proposed narrowing this decision to a
   hybrid: JNI where the payload path is framed and off the frame budget, a
   native codec where the store is mapped. **The re-scope drops the hybrid and
   the JNI half with it**, because once a `ridlc-gen-kotlin` plugin emits a
   Kotlin codec there is no Rust codec left for a Kotlin consumer to reach. The
   plugin generates the whole layer: FlatBuffers readers through the official
   ...
   **C stays dropped as a language target.** ADR-0020 decision 1 brings a C
   ...
   `--wire` concern this paragraph names; its omissions are diagnostics rather
   than silence, and the codec, the constraint checks and the domain types stay
   on the Rust side.
...
15. **The interaction layer is retracted from the language backends, and
    ...
    **Amendment (2026-09-12) — phase 2 has a shape and something to sit on.**
    [ADR-0020](ADR-0020-third-encoding-runtime-layering-and-plugin-system.md)
    decisions 5 to 7 fix what the restored face binds: the ports of the
    `ridl-rt` library, one runtime library per language, with the transport a
    separate package a generated file never names.
    [`docs/wip/2026-09-08-ridl-rt-design.md`](../wip/2026-09-08-ridl-rt-design.md)
    §8 is that face — a `Client` generic over exactly the ports its interface
    needs, and a `Provider` trait the application implements with a generated
    `dispatch` over `Handler`. The predicted convergence on E9.11 does not
    happen: the store and the dispatcher it named are `ridl-engine`'s, parked
    outside this repository, and the face binds ports instead.
...
16. **`ridl-rt` becomes an epic at the ridl layer, in V1.** The runtime core —
    ...
    **Amendment (2026-09-12) — the epic stands; the V1 framing and the subject
    do not.** The runtime is roadmap Epic 11 and sits in step 1 of the two-step
    plan [`docs/ROADMAP.md`](../ROADMAP.md) now runs, which replaces this
    record's V1/V2 vocabulary along with ADR-0004's. The subject narrows to the
    `ridl-rt` library, the two runtimes over it and the frame specification
    ([ADR-0020](ADR-0020-third-encoding-runtime-layering-and-plugin-system.md)
    decisions 5 to 7): the store, the control plane, the sans-IO loop and the
    platform traits listed above are `ridl-engine`'s, parked outside this
    repository and reopened by rmdl. The reason for the epic is unchanged — a
    compiler whose output no runtime can consume is the failure this prevents —
    and E5.12 still describes the rmdl scheduler.
...
17. **The deployment schema is designed first; rsdl is its authoring surface.**
    ...
    **Amendment (2026-09-12) — reversed: rsdl is rewritten directly, and the IR
    is the schema.** The 2026-09-12 re-scope's §3.1 declines this sequencing.
    There is no separate deployment-facts schema step and no TOML descriptor as
    the user-facing surface: rsdl is rewritten around the nouns of
    [`docs/wip/2026-09-08-topology-vocabulary.md`](../wip/2026-09-08-topology-vocabulary.md)
    and lowers to the IR the way ridl does, so the facts listed above become
    part of what rsdl lowers into. The reason given for schema-first — that it
    lets rsdl be designed against something real rather than against a guess —
    is met instead by designing the facts and the syntax together against the
    first consumer's system, which that note's §3.13 states as the acceptance
    test of the rewrite. Roadmap story E11.6, the hand-written deployment-facts
    schema, closes as superseded. The paragraph above on rmdl and the component
    binding contract stands.
...
- **Positive — the release becomes a platform rather than a code generator.**
  Decision 16 puts the runtime in the release, which is what makes the generated
  artifacts consumable by anything. (The V1 wording this bullet carried is
  replaced by the two-step plan; see decision 16's 2026-09-12 amendment.)
- **Positive — one codec, one conformance obligation.** Decisions 4 and 6 mean
  ...
  is discharged once. (Both halves are narrowed by decision 6's 2026-09-12
  amendment and by ADR-0020 decision 1: the Rust codec serves Rust and
  TypeScript, Kotlin's is its plugin's, and three encodings are three
...
7. **Scoping.** Which deployment tiers are targets, and whether Android
   ...
   **Amendment (2026-09-12) — two of the three are settled without the
   scoping.** `repr(C)` is no longer a constrained-tier fallback:
   [ADR-0020](ADR-0020-third-encoding-runtime-layering-and-plugin-system.md)
   decision 1 makes it a payload encoding in its own right, chosen by what a
   consumer can link rather than by a tier. wasm is settled by decision rather
   than by the tier list: ADR-0020 decisions 2 and 7 put the TypeScript codec
   there, though nothing is built yet. What stays open is the tier list —
   whether Android Automotive and Classic AUTOSAR are targets — and the value of
   the optional FFI wrapper of decision 6; [`docs/ROADMAP.md`](../ROADMAP.md)'s
   platform ladder records four rungs and no target in view that needs a fifth.

## Documents amended

| Document                                                         | Change                                                                                                                                                                                                                                                                                   |
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [ADR-0013](ADR-0013-codegen-backend-scope.md)                    | open item 1's interaction-face half answered by decision 15; decision 1's target list superseded by decision 6, and re-superseded 2026-09-12 by ADR-0020 decisions 8 to 11; decision 2's scope made explicit by decision 18 — no service block that projects interactions as RPC methods |
