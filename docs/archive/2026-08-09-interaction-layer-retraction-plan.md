...
**Status: executed 2026-08-09**, in commit `88adafb`. Every task below is done.
Three things the survey found that this plan did not anticipate are recorded at
the end, under "What execution changed"; read that section before treating any
statement above it as current.
...
- [x] Grep for every reference to the emitted vocabulary and to the header
      across `crates/`, `docs/`, and `editors/`: `SignalHandle`, `EventHandle`,
      `Provenance`, `TimingConst`, `ContractStub`, `RidlStream`, `c-header`,
      `CHeader`.
- [x] Record which of `ridl-lsp`'s features depend on the emitted layer as
...
- [x] Check whether any verified fence in `docs/book/` names `c-header` or shows
...
- [x] Write the findings into the PR description before deleting anything.

**Verify:** the survey names every file the later tasks touch.

## Task 1 — Remove the Rust interaction layer

- [x] Delete `crates/ridl-backend-rust/src/interact.rs` and its module
      declaration.
- [x] Delete the interaction snapshots under
...
- [x] Delete the corresponding tests in `crates/ridl-backend-rust/src/tests.rs`,
      including the helpers that exist only for them (`interaction_package`,
      `rust_for_interaction`).
- [x] Confirm `InducedTuple` discovery still works for tuples reached through
...
- [x] Delete `crates/ridl-backend-ts/src/interact.rs` and `src/interact/`,
      including its snapshots and tests.
- [x] Remove the interaction half of the Appendix B TypeScript snapshot.
- [x] Confirm the type-layer snapshots are unchanged — a diff in those means
...
- [x] Delete `crates/ridl-backend-rust/src/c_header.rs` and its snapshots.
- [x] Remove `Emit::CHeader` from `crates/ridlc/src/lib.rs`, the `c-header` CLI
      value, and the branch in `run_build` that writes `<base>.h`.
- [x] Confirm `crates/ridl-ir/src/name.rs` is untouched and still used. ADR-0016
...
- [x] Update the emit tables in `docs/book/getting-started.md` and the `--emit`
      help text.
...
- [x] Add a note to `docs/book/` wherever the generated interaction API is
...
- [x] Close driftsys/ridl#236 as won't-fix, citing ADR-0018 decision 6 — the C
      header's type-name collision cannot occur once the header is gone.
- [x] Leave driftsys/ridl#237 open. The union-arm camel-caser is in
      `crates/ridl-backend-rust/src/lib.rs`, not in the interaction layer, and
      E10's variant naming still walks into it.
- [x] Check whether any remaining diagnostic code became unreachable. RIDL-149
...
**`crates/ridlc/tests/parity.rs` went too — 1461 lines.** The plan did not name
it. It was entirely interaction-scoped: it parsed generated Rust and TypeScript
and asserted both carried the same faces, ordinals, tombstones, timing and
contract stubs, and it existed because three E2 defects were each "one backend
carried an IR fact the other dropped". Nothing it compares survives the
retraction. **Roadmap story E10.11 is added to rebuild it over the type layer**,
which is where E10 gives both backends a new comparable surface.

**Two `services-workspace` proofs went rather than being repaired.**
`services_workspace_composed_compiles_with_rustc` and its TypeScript counterpart
proved cross-package references resolve — but in that corpus those references
came only from interaction signatures and a service's inline shape. With the
layer gone the composed source holds no cross-package reference at all, so both
proofs were duplicates of the `workspace-two-members` pair.

**The `veh-cluster` proofs stayed, with new anti-vacuity markers.** They named
interaction faces; they now name a named scalar, a constant, an enum and a
surviving cross-package reference, so each still refuses to compile nothing.

**Task 0's premise was wrong on one point.** This plan and
[ADR-0018](../decisions/ADR-0018-runtime-core-and-generated-surface.md) both
said the interaction layer was never compiled. It was:
`crates/ridlc/tests/corpus.rs` runs `rustc` over the interaction-bearing corpora
and `tsc` over the TypeScript, each with an anti-vacuity guard. The claim came
from reading two of the four `rustc` tests. The record is corrected; the
retraction rests on the vocabulary being emitted once per package, which
compiling cannot detect.

**Not verified:** `just book-check` never ran, because `mdbook` was not
installed in the environment the work was done in. The book edits were table
rows and prose with no `SUMMARY.md` or `{{#include}}` change, so the risk is low
and unmeasured.
