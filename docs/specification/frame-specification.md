...
> story E11.9; the second is the Kotlin backend's, outside this repository
> (§11).
...
Two bindings are named, by the story or the decision that writes each. Neither
is written yet.
...
### 11.2 AIDL over Binder, the Kotlin backend

Kotlin is the first external plugin of the codegen plugin system, and its IPC
binding is **AIDL over Binder on Android**, generated by the Kotlin backend from
the same lowered model as its types and faces (roadmap, "After step 2 — Kotlin,
the first external plugin";
[the lane P driver](../wip/2026-09-22-lane-p-driver.md) decision D-P5). The
binding is not written in this repository: `ridlc-gen-kotlin` emits it, and a
hand-written Kotlin runtime library presents the ports over it. Its path is
"passed within a node", so its payload encoding is **FlatBuffers** (encoding tag
1), read in place from the parcel through the Kotlin FlatBuffers reader. E11.9's
WebSocket transport is not on Kotlin's path.

What this document fixes for that binding, and where the ridl reference's
Appendix B already points, so that the generated AIDL is a binding of this frame
and not a rival protocol:

- the AIDL **transaction code is the ordinal** (Appendix B, "ordinals"), and an
  AIDL transaction code has a narrower range than `u32` — Binder admits codes
  from `FIRST_CALL_TRANSACTION` to `LAST_CALL_TRANSACTION` — so the binding
  checks every ordinal and interface number against that range at `attach` (§4);
- a signal and an event are delivered through a **callback listener** the
  consumer registers, which is the `subscribe` of §6.2, and a signal's listener
  receives the current state immediately (§5.1);
- a command is an AIDL `oneway` call **plus the acknowledgment** of §5.3 carried
  back through the listener or a second transaction, because `oneway` has no
  confirmation of its own (Appendix B, notes) and the acknowledgment is what
  makes a rejected command a detected event;
- a query is an AIDL method whose reply is the `response` of §5.4, with the
  Stratum 2 outcomes carried through the binding's exception mapping and the
  Stratum 1 error arm inside the reply payload;
- the caller's identity for §7 is the Binder identity of the calling process,
  which Binder supplies to the callee.

What the Kotlin backend needs from the lowered model to generate it is what the
lane P driver's decision D-P4 lists: per package, the interface numbers and
ordinals, each interaction's kind, its payload type and FlatBuffers `MAX_SIZE`,
its timing bounds, its contract clauses, and the catalog hash. Whether a Rust
Binder runtime is in scope for the first Kotlin demo is the roadmap's open item
O-P3.
...
- Roadmap: [`docs/ROADMAP.md`](../ROADMAP.md) — E11.1; E11.9 and "After step 2 —
  Kotlin, the first external plugin" are the two bindings
...
- The Kotlin decision: [the lane P driver](../wip/2026-09-22-lane-p-driver.md),
  decisions D-P4 and D-P5
