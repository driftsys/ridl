...
| RIDL-407 | interaction ordinal changed, or struct field or union arm (typl §7.4) reordered, against the published baseline (§11) — the desk-time drift check, emitted by `ridl check`, never by `ridlc`; a field or arm inserted or removed draws no RIDL-407, and neither does a reorder made in the same edit as an insertion or a removal (`ridl diff` reports both in CI); an enum value's or enum-set bit's reorder is not a change (typl §8, §9) and draws no warning either                                                                                                                                                                                                           | warning  |
