...
limit — the front end builds an expression tree at most 128 nodes high, and an
operator chain raises that height by one level per operator — but it reaches the
IR as canonical text in `Contract.source`, so it adds no message level.
