...
(+ rxdl spellings)
    └─────────┬─────────┘
              ▼
            rsdl             ← architecture: components, instances, placement
```
...
components offer services and require interfaces, and each deployment places
their instances on machines — rsdl never stands alone.
...
| **rsdl Language Reference**    | 0.2 draft | normative           | architecture layer: the system closure, components offering services and requiring interfaces, instances, distributions, deployments and machines, the derived links, crossing kinds and grants; posture derivation deferred                                      |
...
| **Frame Specification**        | 0.1 draft | normative           | the logical frame of the runtime (E11.1): what crosses a boundary per interaction kind in `ridl-rt`'s vocabulary, the control plane, invalid payloads, the rule that a binding is written from it alone; no binding is built                                      |
| **IR Specification**           | 0.1 draft | normative — partial | the IR's encodings and the stability it promises (E4.5a): the canonical encoding and what it fixes, the nesting bound, the compatibility rule, the versioning rule. The plugin protocol (E4.5b) and the diff categories are named there and still owned elsewhere |
...
rxdl spellings); rsdl v0.1 (rewritten as v0.2, archived).
...
13. **Side effects are emissions** — models emit events; binding an event to a
    command is rsdl's, reserved until rmdl is scheduled; nothing in behaviour
    calls anything. _rmdl §5.7, rsdl §12_
...
16. **Models are contract-blind; components situate them** — a model is a pure
    reaction `(O,S)=M(I,S)`; a component offers services and requires
    interfaces, and binding a model to a component's interface members is
    reserved until rmdl is scheduled; a view/model never names its peers. _rmdl
    §7, rsdl §3.2, §12_
17. **Interface is the shape, service is its global published declaration** —
    ...
    components offer services and require interfaces. _ridl §14, rsdl §3.2_
18. **One contract, both postures** — a `service` is posture-neutral, so one
    contract set can ship signal-based (static, bus, Classic) _and_
    service-oriented (discovered, Adaptive). This is the AUTOSAR-vs-SOA/SDV
    reconciliation. Deriving the posture per deployment is deferred; rsdl
    reserves it, and a deployment declares no posture meanwhile. _ridl §14.5,
    rsdl §12_
...
- **Integrator / architect**: everything above at survey depth → rsdl, end to
  end (the system closure, components, placement).
- **Auditor / safety assessor**: this document §3 → typl §1.4 + §7.4 → ridl §10,
  §14 → rmdl §3.2, §6.4, §8 → rsdl §7, §8, §11, §13 → the diagnostics tables of
  each reference.
...
| 27 | ~~**Component = situated reaction**: real-signal boundary via `provides`/`requires`; body is application notation `out = Reaction(in)`; leaf (models) vs composite (sub-components); "composition"/"binding" keywords rejected~~ — **superseded by #36**: a component is one execution context that offers services and requires interfaces; application notation is retired, and binding a reaction to interface members is reserved                                      | rsdl §3.2, §12                         |
| 28 | **Posture derivation**: static (signal/bus, Classic) vs discovered (service, Adaptive) chosen at deployment from static-wire-vs-discovery + physics; one contract deploys both worlds; AUTOSAR vs SOA/SDV reconciled _(derivation later deferred, RSDL-801 and RSDL-803 reserved — see #36)_                                                                                                                                                                               | rsdl §12                               |
| 29 | ~~Redundancy = two components providing one service, **declared** (else build error); shape stays single~~ — **superseded by #36**: one component offers each service (RSDL-502), and redundancy is derived from its instances (RSDL-409)                                                                                                                                                                                                                                  | rsdl §7, §8                            |
| 30 | Bundle collapsed to one concept (spk/apk dropped — Android's, not ours); platform-vs-app is a `tier` attribute _(the bundle is now `distribution` — see #36)_                                                                                                                                                                                                                                                                                                              | rsdl §3.3                              |
...
- **rsdl §17**: dynamic topology/orchestration (elastic) · posture derivation
  **(deferred with RSDL-801 and RSDL-803, rsdl §12; the transport is
  configuration)** · service discovery (versioned services, consumer preference)
  · end-to-end timing composition **(deferred with timing feasibility)** ·
  distribution version ranges and coordinated updates · resilience realization ·
  global service catalog scoping · one owning service per interface across a
  workspace · a member kind without a declaration · what a breaking deployment
  change is. **Answered in v0.2:** the composition/deployment boundary, and
  service-number allocation (the routing key holds no service; a tag-based
  transport reads its own backend key)
...
§11), `RSDL-` (rsdl §16). Of those four, `TYPL-` and `RIDL-` are implemented in
...
| FORM-102 | unexpected token — also the code for a literal the reference grammar does not admit, such as a fractional duration (ridl §2.1), and for nesting past the 128 levels the parser follows: a type, an attribute value, a parenthesised group, or an expression tree, whose height each binary operator, member access, group and prefix raises by one level | error    |
