...
> over its own binder contract, outside this repository (§11.2). The one runtime
> in this repository, `ridl-loopback`, runs in process and speaks no frame, so
> every statement below about delivery, timing enforcement and duplicate
> suppression describes the specification and not a shipped behaviour. See
