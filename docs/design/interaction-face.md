...
against. Four decisions, in the order a reader meets them.

**`write_emits` calls `generate_pipeline`, not `generate`.** The pipeline entry
point emits the face beside the payload types, so `ridl build --emit rust`
produces a package a consumer can link against without a second tool.

**An interface the face cannot carry is skipped, and the rest of the package is
emitted.** `generate_pipeline` walks interfaces one at a time rather than
refusing the package, and leaves a note naming the interface, the reason and the
story that removes the limit. The skip takes that interface's **descriptors**
with it: `single_param_type`, `query_reply_type` and the contract-clause
translator all live in the descriptor emitter and serve both halves, so an
interface whose face cannot be built cannot have its descriptors built either.
The decision as first written assumed the descriptors survive a skip; they
cannot, and the note says what a consumer actually gets. No other interface, and
no payload type, is affected.

**A cross-package reference resolves, and the codec is no longer withheld for
it** (driftsys/ridl#467). `Ctx` carries the other packages of the build, the
codec resolves a reference through them, and — the half the gap did not state —
a foreign owner is written as a path through the module tree `ridlc` writes
rather than as a bare identifier. `docs/design/flatbuffers-codec.md` carries the
detail.

**`check` and every `__ridl_fb_*` function are `pub(crate)`, not `pub`.**
Resolving a cross-package reference means a generated caller may now sit in
another module of the emitted crate, where a private item is unreachable; the
compile proof over the emitted crate fails without this, on the constrained
named scalars a package reaches through the prelude and its imports. The emitted
crate is one crate per build, so `pub(crate)` reaches every generated caller and
adds nothing to the crate's public surface. This deliberately does **not**
settle open item 2 of the 2026-09-20 codec design note — whether `check` is
