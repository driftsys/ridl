...
| `just build`           | `toolchain-check` + `gate-parity` + `install-check` + `fmt-check` + `book-check` + `link-check` + `doc-path-check` + `compile` + `test` + `lint` + `wasm-check` + `compat-check` + `check` — the local gate, which is what CI runs |
